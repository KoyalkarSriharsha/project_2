...
{
  const phno = agent.parameters["phno"][0]+"";
  const client = new MongoClient(url,{ useUnifiedTopology: true });
  await client.connect();
  const snap = await client.db("chatbot_db").collection("personal").findOne({"phno": phno});
  console.log(snap);
  if(snap==null){
...
function report_issue(agent)
{
  ...
  var issue_vals={1:"Internet Down",2:"Slow Internet",3:"Buffering problem",4:"No connectivity",5:"services"};
  
  const intent_val=agent.parameters["number"][0]+"";
  ...
  "richContent": [
    [
      ...
      },
      {
        "type": "list",
        "title": "services",
        "subtitle": "Press '5' for services needed",
        "event": {
          "name": "",
          "languageCode": "",
          "parameters": {}
...
intentMap.set("issues", custom_payload);
intentMap.set("report", report_issue);
